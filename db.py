...
def SoldCard(name, buy_price, sold_price, profit, image, time, amount, pokemon_id):
    #Find the card with the name and pokemon_id and minus the amount by amount,
    collection.update_one({"name": name, "pokemon_id": pokemon_id}, {"$inc": {"amount": -1}})

    #Create a entry in the sold_storage collection
    sold_storage.insert_one({"name": name, "buy_price": buy_price, "sold_price": sold_price,"profit": profit,"image": image , "time": time, "amount": 1})
